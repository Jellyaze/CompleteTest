...
export default function ChatScreen({ route, navigation }: any) {
  ...
  const handleSendID = async () => {
    ...
    try {
      ...
      if (!profile) {
        Alert.alert('Error', 'Profile not found for this user.');
        return;
      }
      ...
      if (hasFront) {
        options.push({
          text: 'Send Front ID',
          onPress: () => sendStoredID(profile.front_id_image_url, 'front'),
        });
      }
      ...
      if(hasBack) {
        options.push({
          text: 'Send Back ID',
          onPress: () => sendStoredID(profile.back_id_image_url, 'back'),
        });
      }
      ...
      if (hasFront && hasBack) {
        options.push({
          text: 'Send Both',
          onPress: () => sendBothIDs(profile.front_id_image_url, profile.back_id_image_url)
        });
      }

      options.push({ text: 'Upload from Gallery', onPress: pickIDFromGallery });
      options.push({ text: 'Cancel', style: 'cancel'});

      Alert.alert('Send ID', 'Which ID would you like to send?', options);
    } catch (error) {
